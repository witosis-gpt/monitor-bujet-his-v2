...
const kepriPlaces = placesForProvince('KEPULAUAN RIAU', master.rates);
assert.ok(kepriPlaces.includes('Batam') && kepriPlaces.includes('Tanjung Pinang') && kepriPlaces.includes('Kab. Bintan'), 'Kepulauan Riau must expose Batam from airline mapping and its SBM ground locations');
const batamFlight = { from: 'JAKARTA', to: 'Batam', fromProvince: 'DKI JAKARTA', toProvince: 'KEPULAUAN RIAU', transportType: 'Flight', flightClass: 'economy' };
const batamEconomy = calculateMovementCosts([batamFlight], team(), [{ city: 'Batam' }], master.rates).find(item => item.rule === 'airline');
assert.equal(batamEconomy.referenceRate, 3091000, 'Jakarta ↔ Batam Economy must use its own SBM airfare');
batamFlight.flightClass = 'business';
const batamBusiness = calculateMovementCosts([batamFlight], team(), [{ city: 'Batam' }], master.rates).find(item => item.rule === 'airline');
assert.equal(batamBusiness.referenceRate, 4867000, 'Jakarta ↔ Batam Business must use its own SBM airfare');
const bintanFlight = calculateMovementCosts([{ from: 'JAKARTA', to: 'Kab. Bintan', fromProvince: 'DKI JAKARTA', toProvince: 'KEPULAUAN RIAU', transportType: 'Flight', flightClass: 'economy' }], team(), [{ city: 'Kab. Bintan' }], master.rates).find(item => item.rule === 'airline');
assert.equal(bintanFlight.active, false, 'Kab. Bintan must not resolve to the Batam airfare');
assert.equal(bintanFlight.lookupError, 'Tarif penerbangan tidak tersedia di master SBM 2026.', 'Kab. Bintan must report its unavailable airfare route instead of substituting Batam');
...
for (const accountCode of ['524111', '524119']) {
  ...
  assert.deepEqual(Array.from(jabodetabek.routeLegs, leg => leg.transportType), ['Ground SBM', 'Ground SBM'], `${accountCode} must default Jakarta ↔ Kab. Bogor to Ground SBM`);
